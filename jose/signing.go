...
func (ctx *genericSigner) Sign(payload []byte) (*JSONWebSignature, error) {
	...
	for i, recipient := range ctx.recipients {
		...
		//serializedProtected = []byte(string(serializedProtected) + "\n")
		needsBase64 := true
		...
		//fmt.Println("input", string(input.Bytes()))
		//log.Println("input", string(input.Bytes()))
